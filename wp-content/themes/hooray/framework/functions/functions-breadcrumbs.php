...
function breadcrumbs_plus_get_items( $args ) {
	...
	/* Link to front page. */
	if ( !is_front_page() )
		$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="' . $args['home'] . '"><meta itemprop="url" content="'. home_url( '/' ) .'"><a href="'. home_url( '/' ) .'" class="home" >' . $args['home'] . '</a></div>&nbsp;/&nbsp;';
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' === $wp_query->post->post_type && $args['show_blog'] ) {
			$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.get_the_title( get_option( 'page_for_posts' ) ).'"><meta itemprop="url" content="'.get_permalink( get_option( 'page_for_posts' ) ).'"><a  href="' . get_permalink( get_option( 'page_for_posts' ) ) . '">' . get_the_title( get_option( 'page_for_posts' ) ) . '</a></div>';
		}
		...
		if ( 'page' !== $wp_query->post->post_type ) {
			...
			if ( function_exists( 'get_post_type_archive_link' ) && !empty( $post_type_object->has_archive ) )
				$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.esc_attr( $post_type_object->labels->name ).'"><meta itemprop="url" content="'.get_post_type_archive_link( $post_type ).'"><a  href="' . get_post_type_archive_link( $post_type ) . '" title="' . esc_attr( $post_type_object->labels->name ) . '">' . $post_type_object->labels->name . '</a></div>';
	...
	/* If viewing any type of archive. */
		...
		if ( is_category() || is_tag() || is_tax() ) {
			...
			if ( is_day() )
				$item['last'] = __( 'Archives for ', LANG ) . get_the_time( 'F j, Y' );

			elseif ( is_month() )
				$item['last'] = __( 'Archives for ', LANG ) . single_month_title( ' ', false );

			elseif ( is_year() )
				$item['last'] = __( 'Archives for ', LANG ) . get_the_time( 'Y' );
		}

		else if ( is_author() )
			$item['last'] = __( 'Archives by: ', LANG ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
	}

	/* If viewing search results. */
	else if ( is_search() )
		$item['last'] = __( 'Search results for "', LANG ) . stripslashes( strip_tags( get_search_query() ) ) . '"';

	/* If viewing a 404 error page. */
	else if ( is_404() )
		$item['last'] = __( 'Page Not Found', LANG );
...
function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
	...
	if ( !empty( $post_type_object->has_archive ) && !bbp_is_forum_archive() ){
		if (function_exists('bp_is_active')) {
			...
			$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.bbp_get_forum_archive_title().'"><meta itemprop="url" content="'.get_post_type_archive_link( bbp_get_forum_post_type() ).'"><a  href="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '">' . bbp_get_forum_archive_title() . '</a></div>';
		}
	}
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
			$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.bbp_get_topic_title( $topic_id ).'"><meta itemprop="url" content="'.bbp_get_topic_permalink( $topic_id ).'"><a  href="' . bbp_get_topic_permalink( $topic_id ) . '">' . bbp_get_topic_title( $topic_id ) . '</a></div>';
		else
			$item[] = '';
	...
	elseif ( bbp_is_single_reply() ) {
		...
		if ( !bbp_is_reply_edit() ) {
			...
			$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.bbp_get_reply_title( $reply_id ).'"><meta itemprop="url" content="'.bbp_get_reply_url( $reply_id ).'"><a href="' . bbp_get_reply_url( $reply_id ) . '">' . bbp_get_reply_title( $reply_id ) . '</a></div>';
			$item[] = __( 'Edit', LANG );
		}
	...
	elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
		...
		if ( bbp_is_single_user_edit() ) {
			$item[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.bbp_get_displayed_user_field( 'display_name' ).'"><meta itemprop="url" content="'.bbp_get_user_profile_url().'"><a href="' . bbp_get_user_profile_url() . '">' . bbp_get_displayed_user_field( 'display_name' ) . '</a></div>';
			$item[] = __( 'Edit', LANG );
		} else {
...
function breadcrumbs_plus_get_parents( $post_id = '', $separator = '/' ) {
	...
	while ( $post_id ) {
		...
		$parents[]  = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.esc_attr( get_the_title( $post_id ) ).'"><meta itemprop="url" content="'.get_permalink( $post_id ).'"><a  href="' . get_permalink( $post_id ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '">' . get_the_title( $post_id ) . '</a></div>';
...
function breadcrumbs_plus_get_term_parents( $parent_id = '', $taxonomy = '', $separator = '/' ) {
	...
	while ( $parent_id ) {
		...
		$parents[] = '<div class="vbreadcrumb" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><meta itemprop="title" content="'.esc_attr( $parent->name ).'"><meta itemprop="url" content="'.get_term_link( $parent, $taxonomy ).'"><a  href="' . get_term_link( $parent, $taxonomy ) . '" title="' . esc_attr( $parent->name ) . '">' . $parent->name . '</a></div>';
