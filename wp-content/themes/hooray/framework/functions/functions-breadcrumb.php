...
function bd_breadcrumb()
{
    ...
    if($bd_data['breadcrumbs']):
        ...
        if ( !is_home() && !is_front_page() || is_paged() )
        {
            ...
            elseif ( is_search() )
            {
                echo $before . __('Search results for ', LANG) . '"' . get_search_query() . '"' . $after;

            }
            elseif ( is_tag() )
            {
                echo $before . __('Posts tagged ', LANG) . '"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() )
            {
                global $author;
                $userdata = get_userdata($author);
                echo $before . __('Articles posted by ', LANG) . $userdata->display_name . $after;

            } elseif ( is_404() )
            {
                echo $before . __('Error 404 ', LANG) . $after;
            }
            if ( get_query_var('paged') )
            {
                if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
                echo __('Page', LANG) . ' ' . get_query_var('paged');
                if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
            }
            echo '</ul>';
        }
