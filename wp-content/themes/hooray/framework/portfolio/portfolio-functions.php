...
if(!function_exists('bd_crumbs')):
    function bd_crumbs() {
        ...
        if ( !is_front_page() ) {
            echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.$data['breacrumb_prefix'].' <a href="';
            echo home_url();
            echo '">'.__('Home', 'bd');
            echo "</a></div>";
        }
        ...
        if (is_category() && !is_singular('wportfolio')) {
            ...
            echo is_wp_error( $cat_parents = get_category_parents($ID, TRUE, '', FALSE ) ) ? '' : '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.$cat_parents.'</div>';
        }

        if(is_singular('wportfolio')) {
            echo get_the_term_list($post->ID, 'portfolio_category', '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">', '&nbsp;/&nbsp;&nbsp;', '</div>');
            echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.get_the_title().'</div>';
        }

        if (is_tax()) {
            $term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
            echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.$term->name.'</div>';
        }

        if(is_home()) { echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.$data['blog_title'].'</div>'; }
        if(is_page() && !is_front_page()) {
            ...
            while ( $parent_id ) :
                ...
                if ( $params["link_none"] )
                    ...
                    $parents[]  = '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_permalink( $page->ID ) . '" title="' . get_the_title( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a></div>' . $separator;
        ...
        if(is_single() && !is_singular('wportfolio')) {
            ...
            if ( $categories ) :
                foreach ( $categories as $cat ) :
                    $cats[] = '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_category_link( $cat->term_id ) . '" title="' . $cat->name . '">' . $cat->name . '</a></div>';
                endforeach;
                echo join( '', $cats );
            endif;
            echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.get_the_title().'</div>';
        }
        if(is_tag()){ echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'."Tag: ".single_tag_title('',FALSE).'</div>'; }
        if(is_404()){ echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.__("404 - Page not Found", 'bd').'</div>'; }
        if(is_search()){ echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.__("Search", 'bd').'</div>'; }
        if(is_year()){ echo '<div class="vbreadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">'.get_the_time('Y').'</div>'; }
