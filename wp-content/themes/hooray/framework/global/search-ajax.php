...
class bd_live_search {
    ...
    function get_templates($template, $type='')
    {
        ...
        switch($type) {
            case 'more':
                if(!$template_post) {
                    $template_post = '<a href="{search_url_escaped}"><span class="sf_text">'.__( 'View All Results', LANG ).'</span></a>';
                }
                break;
            case 'taxonomy':
                if(!$template_post) {
            ...
            case 'post_type':
                if(!$template_post && in_array($template, self::$woocommerce_post_types)) {
                    $template_post = '{post_image_html}<div class="bd-live-search-details"><h3  class="entry-title"><a href="{post_link}">{post_title}</a></h3><div class="bd-live-search-meta-info"><div class="bd-live-search-meta-info-date"><time  class="entry-date updated" datetime="{post_date_formatted}">{post_date_formatted}</time></div></div></div>';
    ...
    function post_object($id, $term_id = false, $matches = false) {
        ...
        if($post != null)
        {
            ...
            if(in_array('{post_image}', $matches) || in_array('{post_image_html}', $matches)) {
                ...
                {
                    $thumb = wp_get_attachment_image_src( $post_thumbnail_id, 'bd-small' );
...
add_filter('sf_posts_query', 'BdLiveSearch_posts_query', 5, 10);
function BdLiveSearch_posts_query($query, $search, $post_type, $excludes, $search_content, $order_results, $limit){
    global $wpdb;
    $wpml_lang_code = (defined('ICL_LANGUAGE_CODE') ? ICL_LANGUAGE_CODE: false);
    if(	$wpml_lang_code ) {
        if(sizeof($excludes) > 0){
...
$bdLiveSearch = new bd_live_search();
?>