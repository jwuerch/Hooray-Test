...
function home2_news_in_tabs_box( $k,$arr ) {
    ...
    if( isset( $wp_cats ) ) { ?>
        ...
        <div class="builder_inner" id="home2_box_<?php echo $k; ?>">
            <input type="hidden" name="home2[<?php echo $k; ?>][type]" value="news_in_tabs_box_home2" />
            <div class="boxes_title" title="News In Tabs Box">
                <i class="handle fa fa-navicon"></i>
                <?php
                    if(is_array($arr['cat']))
                    {
                        $cats_name_bd = array();
                        foreach($arr['cat'] as $bk => $bv)
                        {
                            $cats_name_bd[] =  $wp_cats[$arr['cat'][$bk]];
                        }

                        echo implode(',',$cats_name_bd);
