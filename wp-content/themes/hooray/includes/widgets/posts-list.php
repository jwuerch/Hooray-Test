<?php
add_action( 'widgets_init', 'bdayh_posts_list' );
function bdayh_posts_list() {
    register_widget( 'bdayh_posts_list' );
}

class bdayh_posts_list extends WP_Widget
{
    function bdayh_posts_list()
    {
        $widget_ops = array( 'classname' => 'bd-posts-list', 'description' => 'Widget display Posts order by : Best Reviews Or Recent Posts Or Random Posts Or Last Modified Or Post Name Or Popular Posts / Comments Or Popular Posts / Views' );
        $control_ops = array( 'id_base' => 'bd-posts-list' );
        parent::__construct( 'bd-posts-list', '.: Bdaia - Posts List', $widget_ops, $control_ops );
    }
    ...
    function widget( $args, $instance )
    {
        ...
        ## Widget Posts list
        bdayh_widget_posts_list( $number, $args['widget_id'], $offset, $posts_order, '' );
